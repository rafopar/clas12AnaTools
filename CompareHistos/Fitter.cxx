...
const double Fitter::sqrt2 = 1.41421356237309515;

const double Fitter::X_Offset_2C21 = 36.68;
const double Fitter::Y_Offset_2C21 = 20.97;

const double Fitter::X_Offset_2C24 = 36.68;
const double Fitter::Y_Offset_2C24 = 20.97;

const double Fitter::X_Offset_2H00 = 36.68;
const double Fitter::Y_Offset_2H00 = 20.97;

const double Fitter::X_Offset_2H01 = 36.68;
const double Fitter::Y_Offset_2H01 = 20.97;
...
void Fitter::ParseDataFile(std::string fname) {
    ...
    // =====Initialize these variables, just in case if they are not read from the file.
    avg_2C21_X = 9999.;
    avg_2C21_Y = 9999.;
    avg_2C24_X = 9999.;
    avg_2C24_Y = 9999.;
    ...
    while (getline(dataFile, curLine)) {
        ...
        std::vector<std::string> vecLine(std::istream_iterator<std::string>{iss}, std::istream_iterator<std::string>());

        // ====== The line should have 4 words, "#", "BPM_Name", "X coordinate" and "Y coordinate"
        if (vecLine.size() == 4) {
            if (vecLine.at(1).compare("IPM2C1A_AVG") == 0) {
                avg_2C21_X = atof(vecLine.at(2).c_str());
                avg_2C21_Y = atof(vecLine.at(3).c_str());
                cout << "2C21 Average coordinates are (X:Y) (" << avg_2C21_X << ":" << avg_2C21_Y << ")" << endl;
            } else if (vecLine.at(1).compare("IPM2C4A_AVG") == 0) {
                avg_2C24_X = atof(vecLine.at(2).c_str());
                avg_2C24_Y = atof(vecLine.at(3).c_str());
                cout << "2C24 Average coordinates are (X:Y) (" << avg_2C24_X << ":" << avg_2C24_Y << ")" << endl;
                //                cout << curLine << endl;
            } else if (vecLine.at(1).compare("IPM2H00_AVG") == 0) {
                avg_2H00_X = atof(vecLine.at(2).c_str());
                avg_2H00_Y = atof(vecLine.at(3).c_str());
                cout << "2H00 Average coordinates are (X:Y) (" << avg_2H00_X << ":" << avg_2H00_Y << ")" << endl;
                //                cout << curLine << endl;
            } else if (vecLine.at(1).compare("IPM2H01_AVG") == 0) {
                avg_2H01_X = atof(vecLine.at(2).c_str());
                avg_2H01_Y = atof(vecLine.at(3).c_str());
                cout << "2H01 Average coordinates are (X:Y) (" << avg_2H01_X << ":" << avg_2H01_Y << ")" << endl;
                //cout << curLine << endl;
            }
        }
    }
...
void Fitter::FitData(bool manual_fit, bool preview) {
    ...
    if (counters_box->GetSelected() >= 0) {
        ...
        if (good_parameters) {
            ...
            } else if (fit_2H00A || fit_2H01 || fit_2H02A) // Note: not a very good way, but 2H02A and 2H00A harps are the same harp, therefore it will fit the same method
            {
