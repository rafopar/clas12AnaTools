...
void Draw_calibr()
{
  ...
  const double Imax = 100.; // Max currrent that was used
  const double I2C21Min = 40.; // There is a beam current dependence on the BB attenuation, so when filling the histogram we will measure
                               // the attenuation in the range closer to the production current
  
  const double SLM_slope = 8796.639;
  const double SLM_offset = -5306.;
  //const double SLM_offset = 595.;
  ...
  tr1->ReadFile("Carbon_Nov29_2021_5p99_BBOut.dat", "S2b:S16b:IPM2C21A:IPM2C24A");
  ...
  gr_2c21_Fcup_bbout->Fit(f_lin1, "MeV", "", 4., Imax);
  ...
  TH1D *h_2C21_fCup_ratio_bbin1 = new TH1D("h_2C21_fCup_ratio_bbin1", "", 200, 10., 14.);
  TH1D *h_2C24_fCup_ratio_bbin1 = new TH1D("h_2C24_fCup_ratio_bbin1", "", 200, 10., 14.);
  ...
  tr2->ReadFile("Carbon_Nov29_2021_5p99_BBIn.dat", "S2b:S16b:IPM2C21A:IPM2C24A");
  ...
  for( int i = 0; i < n2; i++ ){
    ...
    if( IPM2C24A*0.04 < IFcup && IPM2C24A*0.09 > IFcup ){
      gr_2c24_Fcup_bbin->SetPoint(ind_2c24, IPM2C24A, IFcup);
      if( IPM2C24A > I2C21Min ){
    ...
    if( IPM2C21A*0.04 < IFcup && IPM2C21A*0.09 > IFcup ){
      gr_2c21_Fcup_bbin->SetPoint(ind_2c21, IPM2C21A, IFcup);
      if( IPM2C21A > I2C21Min ){
  ...
  gr_2c21_Fcup_ratio_bbin->SetMaximum(14.);
  gr_2c21_Fcup_ratio_bbin->SetMinimum(10.);
  ...
  gr_2c24_Fcup_ratio_bbin->SetMaximum(14.);
  gr_2c24_Fcup_ratio_bbin->SetMinimum(10.);
