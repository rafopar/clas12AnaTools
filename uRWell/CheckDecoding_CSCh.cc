...
int main(int argc, char** argv) {
    ...
    const int n_ts = 6;
    const int nSlot = 12; // The number of Hybrids. Their value range rom 0 to 11

    TFile *file_out = new TFile("CheckDecoding_CSCh.root", "Recreate");
    TH2D *h_ADC_chan = new TH2D("h_ADC_chan", "", 1711, -0.5, 1710.5, 400, 2000., 3000.);
    TH2D * h_ADC_Chan_ts_[n_ts];
    TH2D * h_ADC_Chan_slot_[nSlot];
    ...
    for (int islot = 0; islot < nSlot; islot++) {
        h_ADC_Chan_slot_[islot] = new TH2D(Form("h_ADC_Chan_slot_%d", islot), "", 130, -0.5, 129.5, 400, 2000., 3000.);
    }
...
//            }
            ...
            std::map<int, int> m_ADC_slot_[nSlot][n_ts];
            ...
            for (int i = 0; i < n_ADC; i++) {
                ...
                if (layer - uniqueChan / 1000 - 1 != 0 || channel - uniqueChan % 1000 != 0) {
                    //cout << layer - uniqueChan / 1000 - 1 << "   " << channel - uniqueChan % 1000 << endl;
                }

                if (sector == 85) { // Non Sec 6 events are not physical
                    //if (sector == 6) { // Non Sec 6 events are not physical

                    //h_ADC_Chan_slot_[layer]->Fill(channel, ADC);
                    h_ADC_Chan_ts_[ts]->Fill(uniqueChan, ADC);
                    m_ADC_slot_[layer][ts][channel] = ADC;
                    m_ADC_[ts][uniqueChan] = ADC;
                }
            }

            for (auto map0 : m_ADC_slot_[0][0]) {
                int ch = map0.first;

                for (int isl = 0; isl < nSlot; isl++) {
                    double avg_ADC = 0.;
                    for (int i_ts = 0; i_ts < n_ts; i_ts++) {
                        avg_ADC = avg_ADC + m_ADC_slot_[isl][i_ts][ch];
                    }
                    avg_ADC = avg_ADC / double(n_ts);
                    h_ADC_Chan_slot_[isl]->Fill(ch, avg_ADC);
                }
            }
